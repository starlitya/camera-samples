...
import com.android.example.cameraxbasic.opengl.Renderer
import com.android.example.cameraxbasic.opengl.TextureShader
 ...
 */
class AutoFitPreviewBuilder private constructor(config: Preview.Builder,
    ...
    init {
        ...
        useCase = config.build()

        // TODO Jerry
        useCase.setSurfaceProvider SurfaceProvider@{ surfaceRequest ->
            ...
            surfaceRequest.provideSurface(surface, ContextCompat.getMainExecutor(viewFinder.context),
                Consumer<SurfaceRequest.Result> { result ->
                    surface.release()
                    surfaceTexture.release()
                })

            val rotation = getDisplaySurfaceRotation(viewFinder.display)
            updateTransform(viewFinder, rotation, resolution, viewFinderDimens)
        }
    ...
    companion object {
         ...
         * config changes.
         */
        fun build(config: Preview.Builder, viewFinder: TextureView, isFilter: Boolean) =
            AutoFitPreviewBuilder(config, WeakReference(viewFinder), isFilter).useCase
    }
}